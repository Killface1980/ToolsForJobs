...
{
    public class WorkGiver_WorkWithTools_Scanner : WorkGiver_Scanner
    {
        ...
        private static Dictionary<Pawn, Thing> weaponReservations = new Dictionary<Pawn, Thing>()
            {
                {new Pawn(), new Thing()}
            };

        public static Dictionary<Pawn, Thing> WeaponReservations
        {
            get
            {
        ...
        public bool StorageCellExists(Pawn pawn, Thing thing)
        {
            ...
            {
                foreach (var cell in slotGroup.CellsList.Where(cell => cell.IsValidStorageFor(thing) && pawn.CanReserve(cell)))
